...
class ApiAdController extends BaseController
{
    ...
    public function testChangeBossBalance() {
        $user = [15399900130,
                 ...
                 17722687613];
        foreach ($user as $k => $v) {
            $user_id[] = \App\Modules\User\Model\UserModel::select('id')->where('name', $v)->first();
        }

        foreach ($user_id as $k => $v) {
            if (!empty($v)) {
                \App\Modules\User\Model\UserDetailModel::where('uid', $v['id'])->update(['balance' => 0]);
            }
        }
