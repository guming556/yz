...
class ApiCodeController extends BaseController
{
    ...
    public function sendCode( Request $request )
    {
        ...
        // TODO 这样写是因为历史遗留问题，安卓那边用了error，ios用了msg
        if ($is_exist && $code_type == "reg") {
            return response()->json(['message' => '您已经注册过,请登录','error'=>'您已经注册过,请登录'],'500');
        }
        $code      = VerificationModel::sendVerificationCode($code_type, $tel);

        if(is_string($code)){

		    return response()->json( ['error'=>'发送失败,'.$code , 'message'=>'您已经注册过,请登录'] , '500');
        }else{
            return response()->json( ['message'=>'验证码已发送成功,请留意','error'=>'验证码已发送成功,请留意']);
        }
