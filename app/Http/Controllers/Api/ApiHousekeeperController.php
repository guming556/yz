...
//        $ret['bond_price']           = ($ret['conf_list_price'] + $ret['auxiliary_price']) * 0.2;                //工程保证金,两者想加乘以0.2
    ...
    public function getProjectPriceList(Request $request) {
        ...
        if (empty($confList)) return $this->error('找不到设计师配置单');
    ...
    public function housekeeperSubProjectLog(Request $request) {
        ...
        if ($project_type_by_front == 4) return $this->error('非法提交');
...
//          project_type 处于哪个阶段
    ...
    public function getConstructionLog(Request $request) {
        ...
        if (empty($logHandle)) {
            ...
            foreach ($logHandle as $n => $m) {
                foreach ($m as $o => $p) {
                    $m[$o]['evaluate'] = ProjectLogComment::select('project_log_comments.comments', 'user_detail.realname', 'users.user_type')
                        ->leftJoin('user_detail', 'user_detail.uid', '=', 'project_log_comments.uid')
                        ->leftJoin('users', 'users.id', '=', 'project_log_comments.uid')
                        ->where('project_log_id', $p['id'])
                        ->orderBy('project_log_comments.created_at', 'desc')
                        ->first();

                    if (empty($m[$o]['evaluate'])) {
                        $m[$o]['evaluate'] = [];
                    } else {
                        $m[$o]['evaluate']['realname']  = empty($m[$o]['evaluate']['realname']) ? '匿名' : $m[$o]['evaluate']['realname'];
                        $m[$o]['evaluate']['user_type'] = get_user_type_name($m[$o]['evaluate']['user_type']);
                        $m[$o]['evaluate']['comments']  = empty($m[$o]['evaluate']['comments']) ? '' : unserialize($m[$o]['evaluate']['comments']);
                    }
                }
                $last_log['sn']           = $n;
    ...
    public function getConstructionLogAndroid(Request $request) {
        ...
        foreach ($log->toArray() as $key => $value) {
            ...
            //加管家昵称
            $house_keeper_detail = UserDetailModel::select('user_detail.*','users.user_type')->join('users','users.id','=','user_detail.uid')->where('user_detail.uid', $value['house_keeper_id'])->first();
            if (empty($house_keeper_detail)) {
                $nickname = '匿名用户';
            } else {
                $nickname = empty($house_keeper_detail->nickname) ? '匿名用户' : $house_keeper_detail->nickname;
            }
            $value['user_type'] = $house_keeper_detail->user_type;
            $value['nikename']         = $nickname;
            $logHandle[$value['sn']][] = $value;
            ...
            foreach ($logHandle as $n => $m) {
                if (!empty($m)) {
                    foreach ($m as $o => $p) {
                        ...
                        $m_o_evaluate = ProjectLogComment::select('project_log_comments.comments', 'user_detail.realname')
                            ->leftJoin('user_detail', 'user_detail.uid', '=', 'project_log_comments.uid')
                            ->where('project_log_id', $p['id'])
                            ->orderBy('project_log_comments.created_at', 'desc')
                            ->first();
//  var_dump($m_o_evaluate);exit;
                        if (empty($m_o_evaluate)) {
                            $m[$o]['evaluate'] = [];
                        } else {
                            $m_o_evaluate = $m_o_evaluate->toArray();
                            $e_realname = empty($m_o_evaluate['realname']) ? '匿名' : $m_o_evaluate['realname'];
                            $e_comments = empty($m_o_evaluate['comments']) ? '' : unserialize($m_o_evaluate['comments']);
                            $m[$o]['evaluate'][] = ['realname'=>$e_realname,'comments'=>$e_comments];
                        }
                    }
                }
                $last_log['sn']           = $n;
...
//var_dump($logHandle);exit;
        if (empty($is_have_dismantle) && !empty($b['ProjectContent'])) {
            $add_array = [
                "sn" => 4,
                "DailyContent" => []
            ];
            array_unshift($b['ProjectContent'], $add_array);
        }
    ...
    public function postCashUser(Request $request) {
        ...
        if ($res_create_order) {
            ...
            if (curl_errno($ch)) {//出错则显示错误信息
                return $this->error('微信数据获取失败-',-1, curl_error($ch));
            }
...
//                                  ->where('work_offer.status',4)
    ...
    public function withdrawApplication(Request $request) {
        ...
        foreach ($sub_order_id as $o => $p) {
            ...
            if ($sub_order_data->withdraw_status == 1) return $this->errorn('您已申请过提现');
...
//            TODO by HUI 挖坑了，直接判断是否包含某个字符串来判断用那个阶段名称，因为提现的费用说明不想大改动
            ...
            if ($apply_balance > $balance) return $this->error('您申请提现的金额大于账户余额');
