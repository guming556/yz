...
//     2.设计师订单设计师流程中增加上传图片（PC），用户增加查看设计图片。
        ...
        if ($result) {
            return $this->success(['task_id' => $result->id]);
        }
...
//        }
            ...
            if ($value['status'] == 7) {
                ...
                foreach ($work_offer_status as $key2 => $value2) {
                    if ($key2 == count($work_offer_status) - 1) {
                        ...
                        $list[$key]['type_id'] = $value['user_type'];
                        break;
                    }
    ...
    public function payPoundage(Request $request) {
        ...
        if ($difference < 0) {
            // TODO 这里使用第三方支付补缴
            return $this->error(['error' => '余额不足，需使用第三方补缴', 'difference' => abs($difference)]);
        } else {
            // 这里是使用余额支付
    ...
    public function robOrder(Request $request)
    {
        ...
        if (empty($task_info)) return $this->error('订单无法找到！');
        ...
        if (!$result) return $this->error('投稿失败！');
    ...
    public function workConfirmation(Request $request) {
        ...
        if (empty($res_work_submit['status'])) {
            ...
            return $this->error($res_work_submit['successMsg']);
        }

//        return $this->error('提交失败');      这行代码还有必要吗？
...
//        GoodsModel::truncate();
        ...
        if (empty($work_count)) {
            ...
            if ($work_count && empty($work)) {
                ...
                if ($task->save() && $save_work && $sub_order_res && $decrement && $increment_system) {
                    ...
                    return $this->error('取消失败，状态不对','500');
                }
            }
            //业主只确认了人,没付款
            if ($work->status == 1) {
            ...
            if (empty($ret)) return $this->error('找不到该任务', '500');
            ...
            if ($task->status == 7) {
                ...
                foreach ($ret as $key => $value) {
                    ...
                    if ($value['status'] == 0) {
                        ...
                        if ($value['sn'] == 3) {
                            if (empty($data['boss_refuse_reason_id'])) return $this->error('未选择取消原因,无法进行结算', '500');
    ...
    public function getTasks(Request $request) {
        ...
        if ($res_tasks['status']) {
            ...
            return $this->error(['error'=>$res_tasks['errMsg']],500);
        }
    ...
    public function judgeHouseExist(Request $request) {
        ...
        if ($res)
            ...
            return $this->error(['error' => '你需要先预约管家,才能进行监理预约'], 404);
    }
    ...
    public function getDesignerShow(Request $request) {
        ...
        return $this->success(array_values($detail));
    }
    ...
    public function getMerchantDetails(Request $request) {
        ...
        $lat = empty($request->json('lat'))?0:$request->json('lat');
        $lng = empty($request->json('lng'))?0:$request->json('lng');
...
//        if (empty($data_offer)) return response()->json(['error' => '找不到该阶段'], 500);
        if (empty($data_offer_house)) return $this->error('找不到经手人', 500);
...
//                ['avatar'=>1]
//            ];
//            $labor_deatil = json_encode([],JSON_FORCE_OBJECT);
        }
        if(!empty($labor_deatil)){
    ...
    public function bossPaySmallOrderPlanB(Request $request) {
        ...
        if ($should_pay > $boss_detail->balance) {
            return $this->error(['error' => '余额不足，需使用第三方补缴', 'difference' => abs($boss_detail->balance - $should_pay)], 500);
        } else {
            // 这里是使用余额支付
