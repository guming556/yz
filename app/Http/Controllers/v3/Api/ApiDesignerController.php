...
//            $this->payWorkerRespository->bounty($auxiliary_price, $data['task_id'], $data['from_uid'], 1, 6, true);//扣业主冻结资金
    ...
    public function bossRefuseProject(Request $request) {
        ...
        if ($res_refuse && $res_change_labor) return response()->json(['message' => '驳回成功'], 200);
    ...
    public function normalBossRefuseProject(Request $request) {
        ...
        if ($user_type == 4) {
            ...
            foreach ($data_push as $k => $v) {
                $application = 50003;
                $work_data   = UserModel::find($v);
                $message     = $title . PushSentenceList::select('chn_name')->where('nameBelongType', $application)->where('eng_name', 'message_visor_refuse_sub_list')->first()->chn_name;
                $work_data->send_num += 1;
                $work_data->save();
                //保存发送的消息
                save_push_msg($message, $application, $work_data->id);
                PushServiceModel::pushMessageWorker($work_data->device_token, $title . $message, $work_data->send_num, $application);
            }
        } else {
            $task_id = $task_id;
            $data_offer = WorkOfferModel::where('task_id', $task_id)->where('sn', $sn)->first();
            ...
            if (empty($task_visor_data)) {
                ...
                $data_push['house_uid']   = WorkOfferModel::select('to_uid')->where('task_id', $task_id)->where('sn', 0)->first()->to_uid;//管家
                $data_push['super_visor'] = WorkOfferModel::select('to_uid')->where('task_id', $task_visor_id)->where('sn', 0)->first()->to_uid;//监理
            }
            foreach ($data_push as $k => $v) {
                $application = 50002;
                $work_data   = UserModel::find($v);
                $message     = $title . PushSentenceList::select('chn_name')->where('nameBelongType', $application)->where('eng_name', 'message_boss_refuse_sub_list')->first()->chn_name;
                $work_data->send_num += 1;
                $work_data->save();
                //保存发送的消息
                save_push_msg($message, $application, $work_data->id);
...
//        }
     ...
     * 业主查看整改方案
     */
    public function bossGetChangeProjectList(Request $request) {
        $task_id     = $request->json('task_id');//任务id
        $sn          = $request->json('sn');
        $change_data = ProjectLaborChange::where('task_id', $task_id)->where('sn', $sn)->where('is_confirm', 1)->first();
...
//        $ret['bond_price']      = (($ret['conf_list_price'] + $ret['auxiliary_price']) * 0.2);                //工程保证金,两者想加乘以0.2
    ...
    public function houseKeeperProvideDate(Request $request) {
        ...
        if ($res_date && $res_offer){
            ...
            $application = 40012;
/*            $message     = PushSentenceList::select('chn_name')->where('nameBelongType', $application)->where('eng_name', 'message_house_sub_date')->first()->chn_name;
            $woker_info  = UserModel::find($uid);
            $woker_info->send_num += 1;
            $woker_info->save();
            //保存发送的消息
            ...
            push_accord_by_equip($uid, $application, 'message_house_sub_date', '', $task_id);
...
/*            $message   = PushSentenceList::select('chn_name')->where('nameBelongType', $application)->where('eng_name', 'message_sub_list_to_boss')->first()->chn_name;
            ...
            push_accord_by_equip($boss_uid,$application,'message_sub_list_to_boss','',$task_id);
