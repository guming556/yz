...
//     2.设计师订单设计师流程中增加上传图片（PC），用户增加查看设计图片。
        foreach ($data as $key => $value) {
            if (empty($value) && $value != 0) {
                return response()->json(['error' => '工程信息不可为空或工地不存在'], '404');
            }
        }
        ...
        foreach ($data_designer['uid'] as $k => $v) {
            ...
            if (empty($houseKeeperInfo)) {
                return response()->json(['error' => '找不到该管家'], '500');
            }
            if ($houseKeeperInfo->user_type != 3) {
                return response()->json(['error' => '您选择了非管家人员'], '500');
            }
        }
        ...
        if (!($is_work_able['able'])) {
            return response()->json(['error' => $is_work_able['errMsg']], 500);
        } else {
            return response()->json(['task_id' => $is_work_able['successMsg']], 200);
        }
    }
    ...
    public function getTasksAppointHouse(Request $request) {
        ...
        if (empty($user_info)) return response()->json(['error' => '获取不到用户信息'], 500);
...
/*        if ($user_type == 3 && $count_house_keep_robot > 6) {
            return response()->json(['error' => '当前进行中的任务超过6个'], 500);
        }*/

        //抢单监理当前进行中的任务数量是否超过15个，超出不可抢（接）
/*        if ($user_type == 3 && $count_house_keep_robot > 15) {
            return response()->json(['error' => '当前进行中的任务超过15个'], 500);
        }*/
...
/*        if ($user_type == 4) {
    ...
    public function userChangeAppointHouse(Request $request) {
        ...
        if (empty($chang_uid_deatil) || $chang_uid_deatil->user_type !== 3)
            return response()->json(['error' => '找不到该用户或更改的用户不是管家'], 500);
        //先去work表业主第一次选了几个人
        $count_work = WorkModel::where('task_id', $task_id)->count();
        if (empty($count_work)) return response()->json(['error' => '非法操作'], 500);
        ...
        if (empty($origin_id) && $count_work < 3) {
            ...
            if ($result_designer && $result_designer_log) return response()->json(['message' => '更改成功']);
        } else {

            if ($chang_uid == $origin_id) return response()->json(['error' => '您选了相同的设计师,请核对'], 500);
            //修改原来的人work_log
            ...
            if (empty($chang_work)) return response()->json(['error' => '该项目原设计师未找到'], 500);
            //这里要判断下更换的人是否拒绝过这个订单

            //如果原设计师已接受这个订单,不可更改
            $seek_designer_status = workDesignerLog::where('new_uid', $origin_id)->where('is_refuse', 1)->first();
            if ($seek_designer_status) return response()->json(['error' => '该设计师已确认接单,不可更改'], 500);
            ...
            if ($res_change_work && $res_change_work_log) return response()->json(['message' => '更改成功']);
        }

        return response()->json(['error' => '更改失败'], '500');
    }
...
//            $workerStar = UserDetailModel::select('star')->where('uid', $data['to_uid'])->first()->star;
        ...
        if ($work->save() && $workOffer && $task_status) {
            ...
            return response()->json(array('message' => '管家提交报价成功'));
        } else {
    ...
    public function getAuxiliaryBag(Request $request) {
        ...
        return response()->json($auxiliary);
    }
    ...
    public function getConfPriceList(Request $request) {
        ...
        if (empty($conf)) return response()->json(['error' => '找不到配置单'], 500);
        $confList      = unserialize($conf->project_con_list);
        $auxiliaryInfo = DB::table('auxiliary')->where('id', $conf->auxiliary_id)->first();
        if (empty($auxiliaryInfo)) return response()->json(['error' => '辅材包丢失'], 500);
...
//        $ret['bond_price']           = ($ret['conf_list_price'] + $ret['auxiliary_price']) * 0.2;                //工程保证金,两者想加乘以0.2
    ...
    public function getProjectPriceList(Request $request) {
        ...
        if (empty($supervisor_task_data)) return response()->json(['error' => '找不到订单'], 500);
        ...
        if ($user_type_user == 3 && $houseKeeper_list) {
            $task_id = $task_id;
        } elseif ($user_type_user == 4 && $houseKeeper_list) {
        ...
        if (empty($confList)) return response()->json(['error' => '找不到配置单'], 500);
...
//    开工后 管家提交工程日志
    public function housekeeperSubProjectLog(Request $request) {
        ...
        $data['house_keeper_id'] = $request->house_keeper_id;
        foreach ($data as $key => $value) {
            if (empty($value)) {
                return response()->json(['error' => $key . '参数不可为空'], 500);
            }
        }

        $taskInfo = DB::table('task')->where('id', $data['task_id'])->first();
        if (empty($taskInfo)) {
            return response()->json(['error' => '不存在的订单'], 500);
        }
        ...
        if (empty($taskInfo) || empty($work)) response(['error' => '找不到该订单'], 500);
        ...
        if (!empty($offer_data_by_front)) {
            ...
            return response()->json(['error' => '不存在的工程阶段'], 500);
        }
        if ($project_type_by_front == 4) return response()->json(['error' => '非法提交'], 500);
...
//          project_type 处于哪个阶段
    ...
    public function getConstructionLog(Request $request) {
        ...
        if (!empty($taskInfo)) {
            ...
            return response()->json(['error' => '不存在的任务'], 500);
        }
        ...
        if ($taskInfo->user_type != 3 && $taskInfo->user_type != 1) {
            ...
            if (empty($housekeeper_task_info)) {
                return response()->json(['error' => '未有管家'], 500);
            } else {
        ...
        $log = DB::table('project_log')->where('task_id', $task_id)->where('project_position_id', $project_position)->get();
        ...
        foreach ($log as $key => $value) {
            ...
            //加管家昵称
            $house_keeper_detail = UserDetailModel::where('uid', $value['house_keeper_id'])->first();
            if (empty($house_keeper_detail)) {
                $nickname = '匿名用户';
            } else {
                $nickname = empty($house_keeper_detail->nickname) ? '匿名用户' : $house_keeper_detail->nickname;
            }
            $value['nikename']         = $nickname;
    ...
    public function evaluateCreate(Request $request)
    {
        ...
        {
            return response()->json(['message'=>'你不具备评价资格!','code'=>'500']);
        }
        //保存评论数据
        ...
        if($is_evaluate)
        {
            return response()->json(['message'=>'你已经评论过了!']);
        }
        ...
        if(!$result)
        {
            return response()->json(['message'=>'评论失败']);
        }
    ...
    public function getAuxiliaryDetail(Request $request) {
        ...
        if (empty($list_detail)) {
            return response()->json(['error' => '找不到辅材包']);
        }
    ...
    public function getHouseKeeperlists(Request $request) {
        ...
        //实际成交量
//        $data['num_of_employ'] = DB::table('work')
//            ->select(DB::raw('count(uid) as num_of_employ'))
//            ->where('status', '>', 1)
//            ->groupBy('uid')
//            ->get();
        ...
        $data['items'] = UserModel::select('user_detail.nickname', 'user_detail.realname', 'user_detail.city', 'user_detail.star', 'user_detail.avatar', 'user_detail.uid', 'user_detail.experience', 'user_detail.star', 'realname_auth.serve_area', 'user_detail.tag')
        ...
        foreach ($data['items'] as $key => $value) {
            ...
            $num_of_employ                        = UserDetailModel::where('uid', $value['uid'])->first();
            $focus_num                            = UserFocusModel::select('uid')->where('focus_uid', $value['uid'])->count();//关注量
            $data['items'][$key]['num_of_employ'] = !empty($num_of_employ) ? $num_of_employ->employee_num : 0;
            $data['items'][$key]['focus_num']     = $focus_num;
    ...
    public function getFirstOfferPrice(Request $request) {
        ...
        if (empty($task_data) || empty($work_data)) return response()->json(['error' => '找不到该任务的数据'], 500);
        $data_offer = WorkOfferModel::where('task_id', $task_id)->where('work_id', $work_data->id)->where('sn', 0)->first();
        if (empty($data_offer)) return response()->json(['error' => '找不到该任务的数据'], 500);
...
//            $star_house_keeper = UserDetailModel::select('star')->where('uid', $work_data->uid)->first()->star;
    ...
    public function gerProjectDeadTime(Request $request) {
        ...
        if (empty($task_info)) return response()->json(['error' => '找不到该任务的数据'], 500);
    ...
    public function getProjectChangeDeadTime(Request $request) {
        ...
        if (empty($change_date)) return response()->json([]);
        ...
        return response()->json($change_date);
    ...
    public function laborGetProjectInfo(Request $request) {
        ...
        return response()->json($data_guanjia);
    }
    ...
    public function getDelayDates(Request $request) {
        ...
        $task_id   = $request->get('task_id');
        $task_info = TaskModel::find($task_id);
        if (empty($task_info)) return response()->json(['error' => '找不到该任务'], 500);
    ...
    public function getProjectLabors(Request $request) {
        ...
        if (empty($data_labors)) return response()->json(['error' => '系统还未匹配工人'], 500);
        $new_data      = [];
        $labor_rule    = json_decode(ConfigModel::where('type', 'worker')->first()->rule, true);
        $labor_rule[0] = '管家';
        ...
        foreach ($data_labors as $k => $v) {
            ...
            foreach ($workArr as $n => $m) {
                $user_info = UserDetailModel::select('realname_auth.serve_area', 'user_detail.avatar', 'user_detail.nickname', 'user_detail.star', 'user_detail.work_type')
                    ->leftJoin('realname_auth', 'realname_auth.uid', '=', 'user_detail.uid')
                    ->where('user_detail.uid', $m)
//                    ->where('realname_auth.status', 1)
                    ->first();

                if (empty($user_info)) return response()->json(['error' => '找不到工人信息'], 500);
        ...
        return response()->json(array_values($new_data));
    ...
    public function getBillOfTask(Request $request) {
        ...
        $data_all = SubOrderModel::select('order_code', 'cash', 'created_at', 'project_type', 'title', 'task_id', 'fund_state')
            ->where('cash', '>', 0)
            ->where('uid', $uid_boss)
        ...
        if ($data_all->isEmpty()) return response()->json(['error' => '找不到该订单数据'], 500);
        $new_data = [];
    ...
    public function postCashUserAliPay(Request $request) {
        ...
        if (empty($user) || empty(UserModel::find($user_id))) return response()->json(['error' => '找不到该用户,无法充值'], 500);
        ...
        if ($res_create_order) {
            ...
            return response()->json(['orderString'=>($response)]);//就是orderString 可以直接给客户端请求，无需再做处理。
        } else {
    ...
    public function postCashUser(Request $request) {
        ...
        if ($res_create_order) {
            ...
            if (curl_errno($ch)) {//出错则显示错误信息
                return response()->json(['error' => '微信数据获取失败-', curl_error($ch)], 500);
            }
            ...
            return response()->json($params_other);//显示返回信息
...
//        Log::info('数据结构'.json_encode($data));

        $new_data     = $this->xmlToArray($data);
    ...
    public function bossChangeListAnyTime() {
        $str
            = '{
            "7":[
                {
        ...
        $task_id                    = 301;
        $sn                         = 10;
        $offer_data                 = WorkOfferModel::where('task_id', $task_id)->where('sn', $sn)->first();
        $sencond_offer_data         = WorkOfferModel::where('task_id', $task_id)->where('sn', 1)->first();//第二次报价信息
        $work_data                  = WorkModel::where('task_id', $task_id)->where('status', '>', 1)->first();
        $task_info = TaskModel::find($task_id);
        $uid_boss = $task_info->uid;
        $work_origin_price          = $work_data->price;///原阶段work价格
        $project_type               = $offer_data->project_type;//阶段id
        $sencond_offer_origin_price = $sencond_offer_data->price;//该阶段第二次报价的钱
        $offer_change_detail        = create_configure_lists($str)['parent_' . $project_type];//该整改阶段详细


        //算出该阶段是加了钱还是减少钱
        $offer_origin_price = (double)$offer_data->price;//原阶段work_offer价格
        $offer_change_price = (double)$offer_change_detail['parent_price'];//该整改阶段work_offer价格

        $change_money = abs($offer_origin_price - $offer_change_price);//取绝对值

        if ($offer_origin_price > $offer_change_price) {//减少了钱,work和第二次报价相应减少
            $work_data->price          = $work_origin_price - $change_money;
            $sencond_offer_data->price = $sencond_offer_origin_price - $change_money;

        } else {//加了钱,work和第二次报价相应增加
            $work_data->price          = $work_origin_price + $change_money;
            $sencond_offer_data->price = $sencond_offer_origin_price + $change_money;

            //扣款记录(业主)
            //先从工程保证金扣除
            $pretty_cash = $task_info->pretty_cash;
            if ($pretty_cash < $change_money) return response()->json(['error' => '工程保证金不足，需使用第三方补缴', 'difference' => $change_money - $pretty_cash], '500');
            $pretty_cash -= $change_money;
            $task_info->pretty_cash = $pretty_cash;
            //生成子订单
            $is_ordered = OrderModel::sepbountyOrder($uid_boss, $change_money, $task_id, '更改配置单增加的钱');
            //生成财务表
            $financial = [
                'action' => 5,
                'pay_type' => 5,
            ...
            $res_financial = FinancialModel::create($financial);

        }
        /*        $work_data->save();
        /*        $task_info->save();
                $sencond_offer_data->save();*/

        $task_data          = ProjectConfigureTask::where('task_id', $task_id)->where('is_sure', 1)->first();
        $task_origin_detail = unserialize($task_data->project_con_list);//任务原来的总配置单


        $offer_origin_detail = [];//原阶段详细
        $task_change_detail  = [];
        foreach ($task_origin_detail as $item => $value) {
            if ($item == 'parent_' . $project_type) {
                $offer_origin_detail = $value;
            }
            $task_change_detail[$item]                     = $value;
            $task_change_detail['parent_' . $project_type] = $offer_change_detail;
        }


        $new_total_price = '';
        foreach ($task_change_detail as $k => $v) {
            if (!is_numeric($v)) {
                $new_total_price += $v['parent_price'];
            }
        }

        $task_change_detail['all_parent_price'] = $new_total_price;


        //创建数据到project_work_offer_changes表
        $data_create                            = [
            'task_id' => $task_id,
            'project_type' => $project_type,
            'sn' => $sn,
            'offer_origin_price' => $offer_origin_price,//原阶段work_offer价格
            'offer_change_price' => $offer_change_price,//该整改阶段work_offer价格

            'work_origin_price' => $work_origin_price,//原阶段work价格
            'work_change_price' => $work_data->price,//该整改阶段work价格

            'offer_second_origin_price' => $sencond_offer_origin_price,//原阶段管家第二次报价,work_offer价格
            'offer_second_change_price' => $sencond_offer_data->price,//该整改阶段管家第二次报价,work_offer价格

            'offer_change_detail' => serialize($offer_change_detail),//原阶段详细
            'offer_origin_detail' => serialize($offer_origin_detail),//该整改阶段详细
            'task_origin_detail' => serialize($task_origin_detail),//任务原来的总配置单
            'task_change_detail' => serialize($task_change_detail),//该任务变更后的总配置单
        ];
    ...
    public function getLatestChangeListOfProject(Request $request) {
        ...
        if (empty($latest_submit_info)) return response()->json(['error' => '无法找到该管家刚提交的整改单'], 500);
    ...
    public function getLogOfChangeLabor(Request $request) {
        ...
        if ($labors_arr->isEmpty()) return response()->json(['error' => '该任务暂时未更换过工人'], 500);
//        dd($labors_arr->toArray());
        $new_data = [];
    ...
    public function getChangeListLogOfProject(Request $request) {
        ...
        if ($change_task->isEmpty()) return response()->json(['error' => '无法找到整改单'], 500);
        if (empty($origin_task)) return response()->json(['error' => '无法找到配置单'], 500);
        ...
        return response()->json($compare_data);
    }
    ...
    public function getChangeListNowOfProject(Request $request) {
        ...
        if (empty($change_task)) return response()->json(['error' => '无法找到整改单'], 500);
        if (empty($origin_task)) return response()->json(['error' => '无法找到配置单'], 500);
    ...
    public function compareDesignerAndHouserKeeperList(Request $request) {
        ...
        if (empty($designer_task)) return response()->json(['error' => '无法找到设计师的任务'], 500);
        ...
        if (empty($designer_list) || empty($house_keeper_list)) return response()->json(['error' => '无法找到设计师或管家的配置单'], 500);
    ...
    public function laborWorkOfferInfo(Request $request) {
        ...
        if (empty($work_list)) return response()->json(['error' => '无法找到该任务'], 500);
        //拿到所有工人
        $to_uids = WorkOfferModel::where('task_id', $task_id)->where('project_type', '>', 0)->lists('to_uid');
        if ($to_uids->isEmpty()) return response()->json(['error' => '该任务还未分配工人'], 500);
        $to_uids_str        = implode(',', $to_uids->toArray());
        $task_require_labor = strpos($to_uids_str, (string)$to_uid);

        if (empty($task_require_labor)) return response()->json(['error' => '您不在该项目中'], 500);
        ...
        return response()->json($should_do_data);
    ...
    public function endProject(Request $request) {
        ...
        $task_info = TaskModel::find($task_id);
        if (empty($task_info)) return response()->json(['error' => '无法找到该订单'], 500);
        $task_info->status = 9;
        if ($task_info->save()) {
            $user = UserDetailModel::where('uid', $task_info->uid)->first();
            $user->balance += $task_info->pretty_cash;
            $task_info->pretty_cash = 0;
            if ($user->save() && $task_info->save()) return response()->json(['message' => '工程保证金余额已付,订单完结']);
        } else {
            return response()->json(['error' => '结束订单失败'], 500);
        }
    }
...
//                                  ->where('work_offer.status',4)
        ...
        return response()->json($last_work_offer);
    }
    ...
    public function evaluateProject() {
        ...
        $res_create_comment = CommentModel::commentCreate($data);

        switch ($evaluate_star) {
            case 1:
            case 3:
            $score = 20;
                break;
...
/*        $this->initTheme('usertask');//主题初始化
     ...
     * 评价系统(参与人员列表)
     */
    public function getProjectPeopleAll() {

        $task_id = '358';
        //根据管家订单找到设计师id和监理id
        $project_position = TaskModel::find($task_id)->project_position;
        $other_task_uids  = TaskModel::where('project_position', $project_position)->where('status', '<', 9)->where('user_type', 4)->orWhere('user_type', 2)->lists('id');//监理的单和设计师的单子
        if ($other_task_uids->isEmpty()) return response()->json(['error' => '该任务还未分配工人'], 500);

        $designer_and_supervisor = [];
        foreach ($other_task_uids as $k => $v) {
            $designer_and_supervisor[] = WorkModel::select('uid')->where('task_id', $v)->where('status', '>', '1')->first()->toArray();
        }
        //降唯

        $designer_and_supervisor = array_column($designer_and_supervisor, 'uid');
        //找到所有的工人
        $labors_arr = WorkOfferModel::where('task_id', $task_id)->where('project_type', '>=', 0)->lists('to_uid')->toArray();
        //去重
        $all_people = array_merge($labors_arr, $designer_and_supervisor);
        $all_people = array_unique($all_people);
        foreach ($all_people as $k => $v) {
            if (strpos($v, '-')) {
                $ele_and_water = explode('-', $v);
                unset($all_people[$k]);
            }
        }
        $new_all_people = array_merge($all_people, $ele_and_water);
        foreach ($new_all_people as $o => $p) {
            $labors_detail[] = UserDetailModel::select('user_detail.avatar', 'user_detail.nickname', 'user_detail.work_type', 'users.user_type')->where('user_detail.uid', $p)->leftJoin('users', 'users.id', '=', 'user_detail.uid')->first()->toArray();
        }


        foreach ($labors_detail as $k => $v) {
            if ($v['work_type'] == 0) {
                switch ($v['user_type']) {
                    case 2:
                        $work_type_name = '设计师';
            ...
            $labors_detail[$k]['work_name'] = $work_type_name;
            $labors_detail[$k]['avatar']    = url($v['avatar']);
        }
    ...
    public function broadcastList(Request $request) {
        ...
        if ($broadcastList['status']) {
            ...
            return response()->json($task_info_boss);
        } else {
            return response()->json(['error' => $broadcastList['errMsg']], 500);
        }
    }
    ...
    public function withdrawApplication(Request $request) {
        ...
        if(empty($user_work_info)) return response()->json(['error' => '找不到您的信息'], 500);
        foreach ($sub_order_id as $o => $p) {
            $sub_order_data = SubOrderModel::find($p);
            if (empty($sub_order_data)) return response()->json(['error' => '找不到记录,非法提交'], 500);
            if ($sub_order_data->withdraw_status == 1) return response()->json(['error' => '您已申请过提现'], 500);
            if ($uid != $sub_order_data->uid) return response()->json(['error' => '提现人和数据库中不一样,非法提交'], 500);
            ...
            if (empty($user_info)) return response()->json(['error' => '找不到用户'], 500);

            $balance   = $user_info->balance;//找到余额
            $work_info = WorkModel::select('price')->where('task_id', $task_id)->where('status', '>', 0)->first();
            ...
            if (empty($work_offer_data)) return response()->json(['error' => '找不到总价'], 500);
            ...
            if (empty($bank_info)) return response()->json(['error' => '找不到提现账户'], 500);

            $cashout_account = $bank_info->bank_account;
            if (empty($cashout_account)) return response()->json(['error' => '找不到提现银行卡'], 500);

            $taskDetail = TaskModel::find($task_id);
            if (empty($taskDetail)) return response()->json(['error' => '找不到任务'], 500);

            //工地
            $project_position_id   = TaskModel::find($task_id)->project_position;
            $project_position_info = ProjectPositionModel::find($project_position_id);
            if (empty($project_position_info)) return response()->json(['error' => '找不到工地信息'], 500);
            ...
            if (empty($users_info)) return response()->json(['error' => '找不到您的个人资料,无法保存该申请'], 500);

            $work_offer_detail = WorkOfferModel::where('task_id', $task_id)->where('sn', $sn)->first();
            if (empty($work_offer_detail)) return response()->json(['error' => '找不到申请提现的工地阶段'], 500);

            $boss_uid  = $taskDetail->uid;
            $boss_info = UserModel::find($boss_uid);
            if (empty($boss_info)) return response()->json(['error' => '找不到您服务的业主资料'], 500);
            ...
            if ($apply_balance > $balance) return response()->json(['error' => '您申请提现的金额大于账户余额'], 500);
            ...
            $status = DB::transaction(function () use ($data, $apply_balance,$p) {
        ...
        if (is_null($status))
            return response()->json(['message' => '提现申请已提交']);//创建提现申请
        else
            return response()->json(['error' => '提现申请失败'], 500);
    }
    ...
    public function getFinalCashOutResult(Request $request) {
        ...
        if (empty($cash_info)) return response()->json([]);
    ...
    public function withdrawAudit(Request $request) {
        ...
        if (empty($audit_info)) return response()->json(['error' => '找不到该提现申请'], 500);
        $audit_info->status = $status;
        if ($audit_info->save()) return response()->json(['message' => '操作成功']);//创建提现申请
        return response()->json(['error' => '操作失败'], 500);
    }
    ...
    public function confirmWithdraw(Request $request) {
        ...
        if ($res_create_order && $user_info->save()) return response()->json(['message' => '操作成功']);
        return response()->json(['error' => '操作失败'], 500);
    }
    ...
    public function houseKeeperApply(Request $request) {
        ...
        if (HouseKeeperComplaintModel::where('work_offer_id', $work_offer_id)->first())
            return response()->json(['error' => '您已申请过,无需再申请'],500);
        ...
        if(empty($work_offer_data)||empty($task_data)||empty($position_data))
            return response()->json(['error' => '无法找到该项目'],500);
        ...
        if (HouseKeeperComplaintModel::create($data_insert))
            return response()->json(['message' => '发起客诉成功']);
        else
            return response()->json(['message' => '发起客诉失败']);
    }
    ...
    public function houseKeeperDetail(Request $request) {
        ...
        return response()->json($houseKeeper_detail);
    }

    /**
     * 取消管家订单(业主)
     */
    public function bossCancelHouseKeeperOrder() {
        $task_id              = 4;
        $task_info            = TaskModel::find($task_id);
        $boss_uid             = $task_info->uid;//业主id
        $designer_task_info   = TaskModel::where('project_position', $task_info->project_position)->where('status', '<', 9)->where('user_type', 2)->first();
        $supervisor_task_info = TaskModel::where('project_position', $task_info->project_position)->where('status', '<', 9)->where('user_type', 4)->first();
        if (empty($designer_task_info) || empty($supervisor_task_info) || empty($task_info)) return response()->json(['error' => '无法找到设计师或者管家的订单'], 500);
        $designer_task_id   = $designer_task_info->id;//设计师订单id
        $supervisor_task_id = $supervisor_task_info->id;//监理订单id
        dd($designer_task_id,$supervisor_task_id);
        $work_offer         = WorkOfferModel::where('task_id', $designer_task_id)->where('sn', 3)->first();
        if (empty($work_offer)) return response()->json(['error' => '无法找到设计师的施工指导阶段'], 500);
        //拿到设计师施工指导的钱
        $designer_price = $work_offer->price;
        $designer_uid   = $work_offer->to_uid;

        $order_info = OrderModel::where('uid', $boss_uid)->where('task_id', $task_id)->first();//找到唯一的订单编号
        if (empty($order_info)) return response()->json(['error' => '无法找到该订单对应的编号'], 500);
        $only_order_code = $order_info->code;
        ...
        //分阶段进行返钱
        //sn为4-10的时候,找到对应的钱,返给对应的工人
        //那个阶段要扣钱
        $work_offer_data = WorkOfferModel::where('task_id', $task_id)->where('sn', $last_work_offer_status['sn'])->first();//找到哪个阶段
        ...
        $salary                    = $last_work_offer_status['price'];
        $salary += $designer_price;
        //钱不够的话,打回去
        if ($boss_detail_frozen_amount < $salary) {
            return response()->json(['error' => '冻结金不足以支付'], 500);
        }


        //事务处理
        //管家和监理的工资结算,设计师的施工指导阶段结算,工人的工资结算(剩余的20%也要给工人),小订单的结算
        $status = DB::transaction(function () use ($task_id, $boss_uid, $salary, $only_order_code, $title_sn, $boss_detail_frozen_amount, $designer_price, $designer_task_id, $designer_uid,$last_work_offer_status) {
            platformOrderModel::sepbountyOrder($boss_uid, $salary, $task_id, $title_sn . '取消订单', 1);
            $this->payWorkerRespository->bounty($salary, $task_id, $boss_uid, 1, 6);//扣冻结资金
            //找到该用户,扣除余额补充到保证金里面
            $boss_detail_frozen_amount += $salary;
            UserDetailModel::where('uid', $boss_uid)->update(['frozen_amount' => $boss_detail_frozen_amount]);//把扣除金额写进冻结资金
            $this->payWorkerRespository->bounty($designer_price, $designer_task_id, $designer_uid, 1, 2, false, true);//设计师冻结金增加
        });


        dd($last_work_offer_status);
    }
