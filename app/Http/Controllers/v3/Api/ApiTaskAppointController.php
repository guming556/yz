...
//        }
        ...
        //创建一条雇佣记录
        if ($id_employ) {
            return response()->json(['success' => '约单成功', 'id_employ' => $id_employ]);
        }
        return response()->json(['success' => '约单失败'], '500');
    }
    ...
    public function bossChooseDesigner() {
        ...
        if (!empty($user_designer_id)) {
            ...
            if ($result) {
                return response()->json(['success' => '已要求设计师处理您的订单']);
            } else {
            ...
            if ($time_now > strtotime($time_employ_create + 3600 * 24)) {
                ...
                return response()->json(['error' => '订单超过24小时,请重新下单'], '500');
            } else {
                return response()->json(['error' => '系统错误'], '500');
            }
        }
...
//        $pay_to_user_cash = EmployModel::find($id)->bounty;
    ...
    public function interview() {
        ...
        if ($is_accet_res && $employ_res) {
            return response()->json(['success' => '确认设计师成功']);
        } else {
    ...
    public function bossPayDesigner() {
        ...
        if(empty($work)){
            return response()->json(['error' => '该项目状态不对'], '500');
        }
        ...
        if ($is_ordered && $res_pay_designer && $res_frozen_money) {
            ...
            if ($ret_work_status && $ret_offer_sn) {
                ...
                return response()->json(['message' => '支付失败']);
            }
    ...
    public function employBountyUpdate() {
        ...
        if ($employ['employer_uid'] != $data['employer_uid'] || $employ['bounty_status'] != 0) {
            return response()->json(['error' => '该订单已付款'], '500');
        }
        ...
        if (!$is_ordered) return response()->json(['error' => '创建订单失败'], '500');
        //判断用户如果选择的余额支付
        if ($balance >= $employ['bounty'] && $data['pay_canel'] == 0) {
            ...
            if ($password != $alternate_password) {
                return response()->json(['error' => '您的支付密码不正确'], '500');
            }
            //支付产生订单
            $res = EmployModel::employBounty($employ['bounty'], $employ['id'], $data['employer_uid'], $is_ordered->code);

            if ($res) {
                return response()->json(['success' => '支付成功!']);
            } else {
                return response()->json(['error' => '支付失败!']);
            }
    ...
    public function workin() {
        ...
        if ($employ['employer_uid'] == $user_id) {
            ...
            return response()->json(['error' => '参数错误'], '500');
        }
        //查询任务详细
...
//        $designer_id = $request->get('designer_id');     //设计师id
        ...
        foreach ($ret as $key => $value) {
            ...
            if ($value['status'] == 1) {
                ...
                if ($ret_status && $count_submit < 4) {
                    ...
                    //扣款记录(业主)
                    $is_ordered      = ShopOrderModel::employOrder($work['from_uid'], $employ_offer_date['pay_to_user_cash'], $data, $value['title'] . '手续费');
                    ...
                    //收款记录(设计师)
                    $is_ordered_designer = ShopOrderModel::employOrder($work['uid'], $employ_offer_date['pay_to_user_cash'], $data, $value['title'] . '手续费');
    ...
    public function workCreate() {
        ...
        if (!$employ)
            return response()->json(['error' => '你不是被雇佣者不需要交付当前任务稿件'], '500');
        //判断当前稿件是否处于投稿期间
        if ($employ['status'] != 1) {
            return response()->json(['error' => '当前任务不是处于交稿状态'], '500');
        }
        ...
        return response()->json(['success' => '提交成功'], '500');
    }
    ...
    public function acceptWork() {
        ...
        if ($employ['status'] != 2)
            return response()->json(['error' => '当前任务不是处于验收状态'], '500');
        if ($employ['employer_uid'] != $uid)
            return response()->json(['error' => '你不是当前雇佣任务的雇主，不能验收'], '500');
        //dd($employ);
        //验收操作
        $result = EmployModel::acceptWork($id, $uid, false);

        if (!$result)
            return response()->json(['error' => '验收失败'], '500');

        return response()->json(['success' => '你不是当前雇佣任务的雇主，验收成功'], '500');
    }
}
