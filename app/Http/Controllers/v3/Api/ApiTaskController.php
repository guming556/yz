...
//     2.设计师订单设计师流程中增加上传图片（PC），用户增加查看设计图片。
        ...
        if ($count_project_position) return response()->json(['error' => '该地址已存在进行中的任务'], '404');
        ...
        if ($result) {
            return response()->json(['task_id' => $result->id]);
        }
        return response()->json(['error' => '任务发布失败'], '500');
...
//     2.设计师订单设计师流程中增加上传图片（PC），用户增加查看设计图片。
        ...
        foreach ($data_designer['uid'] as $k => $v) {
            ...
            if (empty($houseKeeperInfo)) {
                return response()->json(['error' => '找不到该设计师'], '500');
            }
            if ($houseKeeperInfo->user_type != 2) {
                return response()->json(['error' => '您选择了非设计师人员'], '500');
            }
        }
...
//     2.设计师订单设计师流程中增加上传图片（PC），用户增加查看设计图片。
        ...
        //判断下是否传的是设计师
            ...
            if (empty($houseKeeperInfo)) {
                return response()->json(['error' => '找不到该工作者'], '500');
            }
            if ($houseKeeperInfo->user_type != $data['user_type']) {
                return response()->json(['error' => '选择人员错误'], '500');
            }
        ...
        if (!($is_work_able['able'])) {
            ...
            PushServiceModel::pushMessageWorker($houseKeeperInfo->device_token, $message, $houseKeeperInfo->send_num, $application);
    ...
    public function payPoundage(Request $request) {
        ...
        foreach ($data as $key => $value) {
            if (empty($value)) {
                return response()->json(['error' => $key . '必要参数为空'], '404');
            }
        }
        ...
        if ($difference < 0) {
            // TODO 这里使用第三方支付补缴
            return response()->json(['error' => '余额不足，需使用第三方补缴', 'difference' => abs($difference)], '500');
        } else {
            // 这里是使用余额支付
            ...
            //创建订单
            $is_ordered = OrderModel::bountyOrder($data['uid'], $poundage, $data['task_id'], '任务手续费');

            if (!$is_ordered) {
                return response()->json(['error' => '任务订单创建失败'], '403');
            }
            ...
            if (!$result && empty($res_frozen_money)) {
                return response()->json(['error' => '支付失败'], '403');
            }

            return response()->json(['message' => '任务创建并支付成功']);
        }
    ...
    public function robOrder(Request $request)
    {
        ...
        if (empty($task_info)) return response()->json(['error' => '订单无法找到！'], '403');

        $is_work_able = $this->isWorkAble($data['task_id'], $data['uid']);
        //返回为何不能投标的原因
        if (!$is_work_able['able']) {
            return response()->json(['error' => $is_work_able['errMsg']], '403');
        }
        //创建一个新的稿件
        ...
        if (!$result) return response()->json(['error' => '投稿失败！'], '403');
        return response()->json(['message' => '已抢单，请等待用户确认']);
    }
    ...
    public function getUsersWaitMessage(Request $request)
    {
        ...
        return response()->json($data_message);
    }
    ...
    public function workConfirmation(Request $request) {
        ...
        if (empty($res_work_submit['status'])) {
            return response()->json(['error' => $res_work_submit['errMsg']], '500');
        } else {
            return response()->json(['message'=>$res_work_submit['errMsg']]);
        }
    ...
    public function detectUploadImg(Request $request) {
        ...
        if ($data['sn'] == 1) {
            ...
            if ($workOffer->upload_status == 0) return response()->json(['error' => '请在PC端上传初步设计图纸'], 500);
        ...
        if ($data['sn'] == 2) {
            ...
            if ($workOffer->upload_status == 0) return response()->json(['error' => '请在PC端上传深化设计图纸'], 500);
            return response()->json(['message' => 'succcess']);
        }
    ...
    public function ownerConfirmation(Request $request) {
        ...
        if ($taskUserType == 2) {
            ...
            if ($res_status) {
                ...
                $msg = ['error' => '确认失败', 'count_submit' => $count_submit];
        ...
        if ($taskUserType == 3) {
            ...
            if (empty($data['sn'])) {
                return response()->json(['error' => '缺少参数'], '500');
            }
...
//                ->where('project_type', '>', 0)
    ...
    public function UserCreateChatRooms() {
        ...
        foreach ($new_array as $n=>$m){
            ...
            if(empty($response_data['data']['id'])) return response()->json(['error'=>'失败'],500);
...
//
    ...
    public function ownerFinalConfirmation(Request $request) {
        ...
        if ($all_should_pay->isEmpty()) {
            return response()->json(['error' => '无法找到需要支付给工人的单'], 500);
        }
        $all_pay_price = 0;
        ...
        if (empty($designer_task_info)) return response()->json(['error' => '无法找到设计师订单'], 500);
        ...
        //付钱啦!!
        //扣款记录(业主)
        $status = DB::transaction(function () use ($uid_boss, $all_pay_price, $task_id, $designer_price, $designer_task_id, $designer_uid,$supervisor_task_id,$quanlity_service_money) {
        ...
        if (is_null($status)) {
            ...
            return response()->json(['error' => '确认失败,无法扣款'], 500);
        }
        return response()->json(['message' => '确认成功']);
    }
...
//            ->where('sn', '>', 2)
        ...
        if (empty($ret)) {
            return response()->json(['error' => '未达到结算阶段'], '500');
        }
        $newRetArr = [];
        foreach ($ret as $key => $value) {
            // 处于需要业主确认的阶段才会有这份清单
...
//        GoodsModel::truncate();
    ...
    public function cancelOrder(Request $request) {
        ...
        if (empty($task)) {
            return response()->json(['error' => '找不到该项目'], '500');
        }
        ...
        if (empty($work_count)) {
            ...
            if ($increment && $res_task && $sub_order_res && $decrement_boss_frozen) {
                ...
                return response()->json(['error' => '取消失败']);
            }
        } else {
            // 该项目做到哪个阶段
            ...
            if ($work_count && empty($work)) {
                ...
                $system_id     = 1;
                //收款记录(系统收款)
                $is_ordered_system = OrderModel::sepbountyOrder($system_id, $price_poundage, $data['task_id'], '取消订单手续费(系统账户余额增加)', 2);
            ...
            if (empty($ret)) return response()->json(['error' => '找不到该任务'], '500');
            ...
            if ($task->status == 7) {
                ...
                foreach ($ret as $key => $value) {
                    ...
                    if ($value['status'] == 0) {
                        ...
                        if ($value['sn'] == 3) {
                            if (empty($data['boss_refuse_reason_id'])) return response()->json(['error' => '未选择取消原因,无法进行结算'], '500');
    ...
    public function getTasks(Request $request) {
        ...
        if ($res_tasks['status']) {
            return response()->json($res_tasks['successMsg']);
        } else {
            return response()->json(['error'=>$res_tasks['errMsg']],500);
        }
    ...
    public function designerOrder(Request $request) {
        ...
        return response()->json($total_new);
    ...
    public function judgeHouseExist(Request $request) {
        ...
        if ($res)
            ...
            return response()->json(['error' => '你需要先预约管家,才能进行监理预约'], 404);
    }
    ...
    public function getChangeListHistory(Request $request) {
        ...
        if (empty($task_data)) {
            return response(['error' => '找不到该订单'],500);
        }
        $task_id_real          = FindSuperTaskId($task_id);
        ...
        foreach ($history as $k => $v) {
            $history[$k]['list_detail'] = unserialize($v['list_detail']);
        }
    ...
    public function getProjectList(Request $request) {
        $user_id = $request->json('user_id');
        return response()->json(ProjectPositionModel::getProjectList($user_id));
    }
    ...
    public function getTaskPoundage() {
        ...
        return response()->json(['task_poundage' => $task_poundage]);
    }
    ...
    public function getDesignerShow(Request $request) {
        ...
        $uid_boss = empty($request->get('uid_boss')) ? '' : $request->get('uid_boss');
        $detail   = ShopModel::select(
            ...
            'shop.id as shop_id',
            'users.user_type',
            'users.name as user_mobile',
            'user_detail.cost_of_design'
        )->join('user_detail', 'shop.uid', '=', 'user_detail.uid')
            ->join('users', 'shop.uid', '=', 'users.id')
        ...
        if (!empty($detail)) {
            $detail = $this->taskRespository->WorkersShow($detail,$uid_boss);
        }

        return response()->json(array_values($detail));
    }
...
/*************************************************新约单(整改方案2)*****************************************************************/
    ...
    public function bossApplyOrderPlanB(Request $request) {
        ...
        if (empty($task_info)) return response()->json(['error' => '找不到该任务']);
        $work_info = WorkModel::where('task_id', $data['task_id'])->first();
        if (empty($work_info)) return response()->json(['error' => '找不到经手人']);
        $work_offer_data = WorkOfferModel::where('task_id', $data['task_id'])->where('sn', $data['sn'])->first();
        if (empty($work_offer_data)) return response()->json(['error' => '找不到该阶段']);
        ...
        if (WorkOfferApply::create($data))
            return response()->json(['message' => '发起请求成功']);
        else
            return response()->json(['message' => '发起请求失败']);
    }
...
//        if (empty($data_offer)) return response()->json(['error' => '找不到该阶段'], 500);
        if (empty($data_offer_house)) return response()->json(['error' => '找不到经手人'], 500);
        $task_data     = TaskModel::find($task_id);
...
//            'project_type' => $data_offer->project_type,
    ...
    public function getLatestChangeListOfProjectPlanB(Request $request) {
        ...
        return response()->json($small_order_info);
    ...
    public function getChangeListDataPlanB(Request $request) {
        ...
        if (!empty($small_order['labor'])) {
            $labor_deatil             = UserDetailModel::select('avatar', 'uid', 'users.name', 'user_detail.realname', 'user_detail.work_type')->leftJoin('users', 'users.id', '=', 'user_detail.uid')->where('uid', $small_order['labor'])->first();
            $labor_deatil['avatar']   = url($labor_deatil['avatar']);
            $labor_deatil['realname'] = empty($labor_deatil['realname']) ? '' : $labor_deatil['realname'];
        } else {
            $labor_deatil = [];
        }
        $small_order['labor_deatil'] = $labor_deatil;
        $small_order['total_price']  = $small_order['offer_change_price'] + $small_order['cash_house_keeper'];
        return response()->json($small_order);
    }
    ...
    public function bossPaySmallOrderPlanB(Request $request) {
        ...
        if (empty($small_order_info)) return response()->json(['error' => '系统错误！'], '403');
        if ($small_order_info->status == 3) {
            return response()->json(['error' => '您已支付过该订单！'], '403');
        }
        if (($small_order_info->status == 2) || ($small_order_info->status == 6)) {
            return response()->json(['error' => '非法提交'], '403');
        }
        ...
        if (empty($should_pay)) return response()->json(['error' => '系统错误！'], '403');
        ...
        if ($should_pay > $boss_detail->balance) {
            return response()->json(['error' => '余额不足，需使用第三方补缴', 'difference' => abs($boss_detail->balance - $should_pay)], 500);
        } else {
            // 这里是使用余额支付
...
//        small_order_to_boss($small_order_info->boss_id, 40016);
        ...
        if ($small_order_info->save()) {
            return response()->json(['message' => '提交验收成功'], 200);
        } else {
            return response()->json(['error' => '提交验收失败'], 500);
        }
    ...
    public function houseKeeperReSubmitList(Request $request) {
        $data['sn']                 = $request->json('sn');
        $data['offer_change_price'] = $request->json('cash');//管家提交的该工程需要整改的东西
        $data['cash_house_keeper']  = empty($request->json('cash_house_keeper')) ? 0 : $request->json('cash_house_keeper');//金额(支付管家)
    ...
    public function bossSureSmallOrder(Request $request) {
        ...
        $status = DB::transaction(function () use ($all_price, $task_id, $boss_id, $labor, $title_sn, $project_type,$cash_house_keeper,$house_keeper_id) {
            ...
            //收款记录(工人)
            if(!empty($labor)){
                platformOrderModel::sepbountyOrder($labor, $all_price, $task_id, $title_sn . '工程变更单收入', 1, 1, $project_type);
                $this->payWorkerRespository->bounty($all_price, $task_id, $labor, 1, 2);//工人余额增加
            }

            //收款记录(管家)
            if (!empty($cash_house_keeper)) {
                platformOrderModel::sepbountyOrder($house_keeper_id, $cash_house_keeper, $task_id, $title_sn . '工程变更单收入', 1, 1, $project_type);
        ...
        if (is_null($status)) {
            ...
            return response()->json(['message' => '钱已付工人']);
        } else {
            return response()->json(['error' => '付款失败'], 500);
        }
    ...
    public function cancelSmallOrder(Request $request) {
        ...
        if ($data->status > 2) {
            return response()->json(['error' => '无法取消'], 500);
        }
        $data->status = 7;
