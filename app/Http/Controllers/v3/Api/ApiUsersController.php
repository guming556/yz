...
class ApiUsersController extends BaseController
{
    ...
    public function register(Request $request){
        ...
        if( $data['user_type'] === '1' ){
            ...
            if ($data['user_type'] == 2) {
                ...
                $levelInfo        = DB::table('level')->where('type', 1)->first()->upgrade;//监理1星级对应的分数
                $upgrade          = json_decode($levelInfo, true);
                $data['workStar'] = 1;
                $data['score']    = $upgrade[1];
            } else {
    ...
    public function getChatRoom(Request $request) {
        ...
        //不是业主的话
        $user_type = UserModel::find($user_id)->user_type;
        if ($user_type != 1) {
    ...
    public function saveDeviceToken(Request $request) {
        ...
        $user_info   = UserModel::find($uid);
        if (empty($user_info)) return response()->json(['error' => '找不到该用户'], '500');
        $user_info->device_token = $deviceToken;//保存机器码
        if ($user_info->save()) {
            return response()->json(['message' => 'success'], 200);
        };
...
/*        //通知
        ...
        $deviceToken = '971b5410962c8463f29e1e511cc4070628467d7ea1bfdb0b98549864cbecc2d1';//应用对应机器码(去掉空格)
...
//    根据条件筛选
    ...
    public function GenInvitationCode(Request $request) {
        ...
        $url           = $host . '/api/registerPlatform/' . $code;
    ...
    public function postRegisterByCode(Request $request) {
        $invite_code           = empty($request->code) ? 0 : $request->code;
        ...
        if (!$checkCode) {
            return back()->withErrors(['messageOfStatus' => '验证码不正确!']);
        }

        UserModel::createUserMobile($data);

        $this->chatRoomRespository->RegistEaseMob($data['username']);
        return back()->withErrors(['messageOfStatus' => '注册成功!']);
    }
    /**
     * @param Request $request
     * 获取线下转账的公司信息
