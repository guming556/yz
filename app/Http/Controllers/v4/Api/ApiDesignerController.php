...
class ApiDesignerController extends BaseController {
    ...
    public function workRelease(Request $request) {
        // 主要
        $data['uid']          = $request->input('user_id');      //用户id
        $data['title']         = $request->input('title');        //标题
        $data['style_id']      = $request->input('style_id');     //风格id
        $data['square']        = $request->input('square');       //面积
        $data['house_id']      = $request->input('house_id');     //户型id
        $data['cover']         = $request->file('cover');       //封面图
        $data['goods_address'] = $request->input('goods_address');       //作品地址
        $data['status']        = 1;
        ...
        if ($goods) {
            return $this->success(array('object_id' => $goods->id));
        }
    ...
    public function workAttachment(Request $request) {
        ...
        $data['object_id'] = $request->input('object_id');      //发布的作品id
        $data['picture']   = $request->file('picture');       //图片
        $data['user_id']   = $request->input('user_id');       //用户id
        $data['title']     = $request->input('title');       //位置名称
        $data['desc']      = $request->input('desc');       //描述

        foreach ($data as $key => $value) {
            if (empty($value)) {
                return $this->error('参数不可为空');
            }
        }
...
//            $value['cover']  = !empty($value['cover']) ? url($value['cover']) : '';
    ...
    public function designerOffer(Request $request) {
        ...
        if (empty($change_unit_price)) return $this->error('保存单价失败');
    ...
    public function getHouseKeeperPrice(Request $request) {
        ...
        if (empty($uid_houseKeeper)) return $this->error('该管家还没中标');
        ...
        if ($star_house_keeper > $star_select) {
            return $this->error('您选择的星级大于您本身的星级');
        }
        ...
        return $this->success(['salary' => $salary]);
    }
    ...
    public function bossPayHouseKeeperOffer(Request $request) {
        ...
        if (empty($data_lists)) return $this->error('找不到该配置单');
        ...
        $work_off_data = WorkOfferModel::where('from_uid', $data['from_uid'])
            ...
            ->where('sn', 1)->first();
        if ($work_off_data->status == 4) return $this->error('您已支付过该订单');
        ...
        if ($user_boss->balance < ($total_salary_all)) {
            return $this->error('账户余额不足',-3,['difference' => abs($user_boss->balance - ($total_salary_all))]);
        } else {
            // 这里是使用余额支付
...
/*            $message     = PushSentenceList::select('chn_name')->where('nameBelongType', $application)->where('eng_name', 'message_boss_pay_list')->first()->chn_name;
    ...
    public function bossPayHouseKeeperSalary(Request $request) {
        ...
        if (empty($work_detail)) return $this->error('系统错误');
        if ($work_detail->status == 2) return $this->error('您已支付过该订单');
        $task_info = TaskModel::find($task_id);
        ...
        if ($task_info->user_type == 4) {
            ...
            if ($salary > $boss_detail->balance) {
                return $this->error('余额不足，需使用第三方补缴',-1,['difference' => abs($boss_detail->balance - $salary)]);
            }
        }
   ...
   /*         $work_data = UserModel::find($worker_id);
    ...
    public function bossRefuseProject(Request $request) {
        ...
        if (empty($curSnStatus)) return $this->error('当前阶段已验收,无法驳回');
        if ($curSnStatus[0]['sn'] != $sn) return $this->error('非法提交');
...
//            ->where('project_type', '>', 0)
        ...
        if (empty($curSnStatus)) return $this->error('当前阶段已验收或已处于驳回阶段 , 无法驳回');
        if ($curSnStatus[0]['sn'] != $sn) return $this->error('非法提交');
...
//        }
    ...
    public function rejectDelayReport(Request $request) {
        ...
        if ($updateRet) {
            return $this->error('驳回工程延期单成功',0);
        }
        return $this->error('驳回工程延期单失败');
    }
    ...
    public function rejectListChangeReport(Request $request) {
        ...
        if ($updateRet) {
            return $this->error('驳回工程整改单成功',0);
        }
        return $this->error('驳回工程整改单失败');
    ...
    public function bossSureDelayDate(Request $request) {
        ...
        if ($data_delay->save() && $res_sure_date && $res_offer) return $this->error('已确认延期时间',0);

        return $this->error('确认延期时间失败');
    }
    ...
    public function bossSureChangeList(Request $request) {
        ...
        if ($updateRet && $user_type == 1) {
            return $this->error('确认成功,平台正在匹配中',0);
        } elseif ($updateRet && $user_type == 4) {
            return $this->error('确认成功,待业主确认',0);
        }

        return $this->error('确认工程整改单失败');
    ...
    public function houseKeeperReChangeList(Request $request) {
        ...
        $end_date   = $request->json('end_date');
        var_dump($request->json('sn'),$task_id);exit();
        $data_offer = WorkOfferModel::where('task_id', $task_id)->where('sn', $sn)->where('status', 3)->first();
        if (empty($data_offer)) return $this->error('该阶段未处于驳回状态');
        ...
        if (empty($labor_change_data)) return $this->error('找不到此次业主驳回记录');
        ...
        if ($new_total_price > $old_total_price) {
            return $this->error('该整改阶段设置参数计算的总价大于原阶段的总价');
        }
        if ($labor_change_data->save()) return $this->error('提交成功',0);
    ...
    public function bossGetReChangeList(Request $request) {
        ...
        if (empty($data_labor)) return $this->error('找不到此次业主驳回记录');
    ...
    public function bossRejectLists(Request $request) {
        ...
        $data_labor->status = 3;
        if ($data_labor->save()) return $this->error('驳回成功',0);
        return $this->error('驳回失败');
    }
...
//        small_order_to_worker($user_super, 30002, 'message_customer_complaint');
        if ($data_labor->save()) return $this->error('确认成功,平台正在匹配中',0);
...
//        var_dump($data['task_id']);exit();
        if ($taskInfo->user_type == 2) {
            $conf = ProjectConfigureTask::where('task_id', $data['task_id'])->orderBy('id', 'desc')->first();//拿到配置单
//            var_dump(unserialize($conf->project_con_list));exit();
            if (empty($conf)) return $this->error('该任务找不到对应的工程配置单');
...
//var_dump($confList);exit();
    ...
    public function seekConfTotalPrice(Request $request) {
        ...
        if (empty($conf)) return $this->error('找不到该配置单');
...
//        $ret['bond_price']      = (($ret['conf_list_price'] + $ret['auxiliary_price']) * 0.2);                //工程保证金,两者想加乘以0.2
    ...
    public function bossSureLists(Request $request) {
        ...
        //TODO
//        $work_offer_data = WorkOfferModel::where('task_id', $task_id)->where('sn', 1)->whereIn('status', [1,1.5])->update(['status' => 2]);

        /*        //管家报价阶段,单独拿出来,不需要监理确认
                if (($curSnStatus['status'] == 1 || $curSnStatus['status'] == 3) && $curSnStatus['sn'] == 1) {
                    if ($userInfo != 1) {
                        return $this->error('不是业主，无法操作');
                    } else {
                        ...
                        if ($res_modify_status && $res_count) return $this->error('业主确认验收成功');
                        return $this->error('业主确认验收失败');
                    }
                }
...
/*           $message     = PushSentenceList::select('chn_name')->where('nameBelongType', $application)->where('eng_name', 'message_sub_list_to_boss')->first()->chn_name;
    ...
    public function houseKeeperProvideDate(Request $request) {
        ...
        $task_id    = $request->json('task_id');
//        var_dump(strtotime($start_date) , strtotime($end_date));exit;
        if(strtotime($end_date) < strtotime($start_date)){
            return $this->error('竣工时间不可小于开工时间');
        }
...
//            small_order_to_boss($uid,40012,'message_house_sub_date');
            ...
            return $this->error('提交时间成功',0);
        }
        return $this->error('提交时间失败');
    }
...
//        return $work_type;
        ...
        if ($count <= 1) {
            return $this->error('您只提交了水电阶段的泥水工或水电工项目,请补全配置单');
        }
        ...
        $workInfo = WorkModel::select(
            ...
            ->first();
        if (empty($workInfo)) return $this->error('找不到该管家订单');
        $workInfo           = $workInfo->toArray();

        $designer_task = TaskModel::where('project_position', $task_info->project_position)->where('status', '<', 9)->where('user_type', 2)->first();//119
        ...
        if (empty($confList)) return $this->error('找不到设计师提交的配置单');
...
//            small_order_to_boss($boss_uid,40010,'message_sub_list_to_boss');
            ...
            if ($is_have_price) {
                ...
                if ($is_have_price->save()) return $this->error('提交成功');
                return $this->error('提交失败');
            } else {
                $workOfferInsertArr = [
                    'type' => 'housekeeper',
                ...
                if (WorkOfferModel::create($workOfferInsertArr)) return $this->error('提交成功');
                return $this->error('提交失败');
            }
        } else {
    ...
    public function getListDetailBysn(Request $request) {
        ...
        if (empty($data_lists)) return $this->error('找不到该数据');
    ...
    public function designerDetail(Request $request) {
        ...
        if (empty($designer_id)) return $this->error('非法参数');

        $detail = $this->userRespository->getDesignerDetailByid($designer_id,$user_id);

        return $this->success(empty($detail) ? [] : $detail);
    }
    ...
    public function searchDesigner(Request $request) {
        ...
        return $this->success(empty($list) ? [] : $list);
    }
    ...
    public function getDesignerImg(Request $request) {
        ...
        if (empty($work_id)) return $this->error('无法找到该任务');
...
//        $imgList = WorkAttachmentModel::select('attachment.name', 'attachment.url')->where('img_type',1)->where('task_id', $task_id)->where('work_id', $work_id->id)->leftJoin('attachment', 'work_attachment.attachment_id', '=', 'attachment.id')->get();
        ...
        if ($workOffer['upload_status'] == 0) return $this->error('设计师仍未提交图纸');
...
//        if ($imgList->isEmpty()) return $this->error('设计师仍未提交图纸');
        if ($workOffer['upload_status'] == 0) return $this->error('设计师仍未提交图纸');
