...
//            return $this->error('不能低于最小限定金' . $task_bounty_min_limit);
//        }
        //创建一条雇佣记录
        $data['employee_uid']      = intval($data['employee_uid']);
        $data['employer_uid']      = intval($data['employer_uid']);
        ...
        //创建一条雇佣记录
        if ($id_employ) {
            return $this->error(['success' => '约单成功', 'id_employ' => $id_employ]);
        }
...
//        $pay_to_user_cash = EmployModel::find($id)->bounty;
        ...
        if ($square <= 0) return $this->error('参数不正确！', 500);
        if ($unit_price <= 0) return $this->error('参数不正确！', 500);
