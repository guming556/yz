...
class ApiUsersController extends BaseController
{
    ...
    public function rule( $validatorData = array() ){
        $rule = Validator::make(
            $validatorData,
            [
                'username' => 'size:11|unique:users,name,null,id',
...
//                'username.mobile_phone'   => '请输入一个手机号码',
    ...
    public function register(Request $request){
        ...
        if ($validator->fails()){
            return $this->error($validator->messages()->first() , 403);
        }
        ...
        foreach ($data as $key => $value) {
            if(empty($value)){
                return $this->error($key.'注册资料不完整' , 403);
            }
        }
        ...
        if( $data['user_type'] == 1 ){
            ...
            if(!$checkCode){
                return $this->error('验证码不正确', 403);
            }

            if ( UserModel::createUserMobile( $data ) ){
                VerificationModel::sendWelcomeMsg($data['username']);
                return $this->success( array('username'=>$data['username'],'user_type'=>$data['user_type'],'status'=>1) );
            }
            ...
            $serve_area_id = CoordinateModel::where('name','like','%'.$realnameInfo['serve_area'].'%')->where('level',2)->orderBy('level','desc')->first();
...
//            var_dump($data);exit;
            ...
            if(UserModel::createUserMobile( $data , true , $realnameInfo )){
                return $this->success( array('username'=>$data['username'],'user_type'=>$data['user_type'],'status'=>0));
            }
        }

        return $this->error('注册失败或下一步失败'  , 500);
    }
    ...
    public function getChatRoom(Request $request) {
        ...
        return $this->success($all_chat_room);
    }
    ...
    public function login(Request $request){
        ...
        if($loginTime){
            ...
            return $this->success(['token'=>$token,'userInfo'=>$useInfo->toArray()]);
        }
    ...
    public function logout(Request $request) {
        ...
        if (empty($user_info)) return $this->error('找不到该用户');
    ...
    public function saveDeviceToken(Request $request) {
        ...
        //判断是ios端还是安卓端
//        var_dump($user_info->toArray());exit;
        $step = '没有，不推送';
            ...
            if ($deviceToken[1] != $now_token_type) {  //原机器码和传过来的机器码不相等
                ...
                if($android_to_android){
                    ...
                    if ($user_type == 1) {
                        ...
                        android_push_to_worker($uid, 50009,'message_change_token_work');
                    }
                }else{
                    if ($user_type == 1) {
                        //推送给业主
                        ...
                        //推送给工作者
                        small_order_to_worker($uid, 50009, 'message_change_token_work');
                    }
                }
...
//            }
     ...
     */
    public function worksLists(Request $request) {
...
//        $minus_lng = $lng - 0.1;
        ...
        $recommend = ShopModel::select('user_detail.avatar', 'user_detail.uid', 'user_detail.nickname', 'users.name')
            ->where('shop.is_recommend', 1)
            ->where('shop.status', 1)
            ->where('users.status', 1)
            ->where('users.user_type', 2);

        $items = GoodsModel::select('goods.id', 'goods.goods_address', 'goods.view_num', 'goods.title', 'goods.cover', 'goods.uid', 'user_detail.avatar', 'user_detail.receive_task_num')
            ->where('goods.status', 1)
            ->where('goods.type', 1)
            ->where('is_delete', 0);

        if(!empty($userCityId)){
            $data['recommend'] = $recommend->leftJoin('user_detail', 'user_detail.uid', '=', 'shop.uid')
                                            ->leftJoin('users', 'user_detail.uid', '=', 'users.id')
                                            ->where('user_detail.serve_area_id',$userCityId)
                                            ->limit(3)
                                            ->get()->toArray();

            $data['items'] = $items->leftJoin('user_detail', 'user_detail.uid', '=', 'goods.uid')
                ->leftJoin('users', 'user_detail.uid', '=', 'users.id')
                ->where('user_detail.serve_area_id',$userCityId)
                ->where('users.user_type',2)
                ->orderBy('goods.updated_at', 'desc')->paginate(10)->toArray()['data'];
     ...
     * 获取所有设计师
     */
    public function designerLists(Request $request){
...
//        $minus_lng = $lng - 1;
        ...
        $recommend = ShopModel::select('user_detail.avatar','user_detail.uid','user_detail.nickname','users.name','user_detail.lat','user_detail.lng')
            ->where('shop.is_recommend',1)
            ->where('shop.status',1)
            ->where('users.status',1)
            ->where('users.user_type',2);

        $items = ShopModel::select('user_detail.nickname','user_detail.realname','user_detail.city','user_detail.address','user_detail.cost_of_design','user_detail.avatar','user_detail.uid','user_detail.experience','user_detail.receive_task_num')
            ->where('shop.status',1)
            ->where('users.status',1)
            ->where('users.user_type',2);
        ...
        if(!empty($userCityId)){
            ...
            $data['items'] = $items->leftJoin('user_detail' , 'user_detail.uid','=','shop.uid')
                ->leftJoin('users','user_detail.uid','=','users.id')
...
//                //浏览量
    ...
    public function payDesignerPrice(Request $request) {
        ...
        if ($work['status'] == 2) return $this->error('该订单已支付');
        ...
        if ($diff < 0) {
            // TODO 这里使用第三方支付补缴
            return $this->error('余额不足，需使用第三方补缴', -1,['difference' => abs($diff)]);
        } else {
            // 这里是使用余额支付
        ...
        if (empty($origin_order)) return $this->error('找不到该订单',500);
...
/*                    $work_data   = UserModel::find($work->uid);
                ...
                return $this->error('步骤一支付失败');
            }
            return $this->error('步骤二支付失败');
        }

        return $this->error('步骤三支付失败');
    }
    ...
    public function getUserBalance(Request $request) {
        ...
        $uid = $request->get('uid');
        if (empty($uid)) {
            return $this->error('uid为空');
        } else {
//            $balance       = UserDetailModel::where('uid', $uid)->first()->balance;
//            $frozen_amount = UserDetailModel::where('uid', $uid)->first()->frozen_amount;
//            $frozen_amount = UserDetailModel::where('uid', $uid)->first()->frozen_amount;

            $info       = DB::table('user_detail')->where('uid', $uid)->first();

            return $this->success(['balance' => number_format($info->balance, 2), 'frozen' => number_format($info->frozen_amount, 2)]);
        }
    }
    ...
    public function getRefuseReason() {
        ...
        return $this->success($data_reason);
    }
...
//    根据条件筛选
    ...
    public function getEvaluateOfWorker(Request $request) {
        $uid = $request->get('uid');
        //头像,姓名,头像,评价id
        $comments = CommentModel::select('users.name','comments.from_uid', 'comments.total_score', 'comments.comment', 'comments.created_at', 'user_detail.avatar', 'user_detail.realname', 'user_detail.nickname')
...
//            $boss_name = empty($v['nickname']) ? $v['realname'] : $v['nickname'];

//            $is_num = is_numeric($v[]);
//            if($is_num){
                $comments[$k]['boss_name'] = substr_replace($v['name'],'****',3,4);
//            }else{
//                $comments[$k]['boss_name'] = empty($v['nickname']) ? $v['realname'] : $v['nickname'];
//            }
...
//    var_dump($res_comment);exit();
//        if (!empty($res_comment)) {
//            return $this->error('您已经评论过了');
//        }
...
//            return $work_offer_info->id;exit();
    ...
    public function evaluateStatus(Request $request) {
        ...
        if (empty($work_offer_data))
            ...
            return $this->success(['status' => $work_offer_data->evaluate_status]);
    }
    ...
    public function evaluateStatusOfHouser(Request $request) {
        ...
        if (empty($house_work_info)) return $this->error('系统错误');
        //默认取报价阶段的状态判断有没有评价过
    ...
    public function getUserBankInfoByid(Request $request) {
        ...
        return $this->success($uid_bank_info);
    }
    ...
    public function postRegisterByCode(Request $request) {
        ...
        if (!(isMobile($data['username']))) {
            return $this->error('您输入的不是手机号码',500);
        }
...
//    }
    ...
    public function getHousePriceByStar(Request $request) {
        ...
        return $this->success($workerStarPrice);
    }
     ...
     *
     */
    public function getCoordinateAndroid(Request $request) {
        $list = CoordinateModel::findTreeAndroid();
        return $this->success($list);
    }
    ...
    public function getScreenCity(Request $request) {
        $list[] = [
            "id"=> 0,
            "name"=>"全部",
            "lat"=> "0",
        ...
        $workcitylist = CoordinateModel::workCity();
        foreach($workcitylist as $key => $value){
            $list[] = $value;
        }
