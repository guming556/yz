...
class AdController extends ManageController
{
    ...
    public function adlist(Request $request){
        ...
        $by = $request->get('by') ? $request->get('by') : 'listorder';
    ...
    public function newStoreAdInfo(Request $request){
...
//            'ad_url.required' => '请输入链接',
    ...
    public function newUpdateAdInfo(Request $request,$ad_id){
        if(!$ad_id){
...
//            'ad_url.required' => '请输入链接',
    ...
    public function changeAdState($ad_id){
        $adInfo = AdModel::find($ad_id);
        if(empty($adInfo)){
            return redirect('/advertisement/adList')->with(['error'=>'传送参数错误！']);
        }
        $res = $adInfo['attributes']['is_open'] == 1? $adInfo->update(['is_open' => '2']) : $adInfo->update(['is_open' => '1']);
        if($res){
            return redirect('/advertisement/adList')->with(['message'=>'操作成功！']);
        }
        else{
            return redirect('/advertisement/adList')->with(['message'=>'操作失败！']);
        }
    }

    public function getInsertAd()
    {
        $this->theme->setTitle('广告添加');
    ...
    public function allDelete(Request $request)
    {
        $data = $request->except(['_token','_url']);
        // var_dump($data);exit;
        $res = AdModel::destroy($data);
        
        if ($res) {
            return redirect()->to('/advertisement/adList')->with(array('message' => '操作成功'));
        }
        return redirect()->to('/advertisement/adList')->with(array('message' => '操作失败'));
    }
