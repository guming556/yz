...
Route::group(['prefix' => 'advertisement', 'middleware' =>[ 'manageauth', 'RolePermission']], function () {
	...
	Route::post('/adInfo','AdController@newStoreAdInfo')->name('adCreate');
	Route::get('/update/{id}','AdController@getUpdateAd')->name('adUpdatePage');
	Route::post('/updateInfo/{id}','AdController@newUpdateAdInfo')->name('adUpdate');
	Route::get('/deleteInfo/{id}','AdController@deleteAdInfo')->name('adDelete');
    Route::get('/changeState/{id}','AdController@changeAdState')->name('adChangeState');
    Route::post('/allDelete', 'AdController@allDelete')->name('allDete');
