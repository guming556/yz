...
class EmployModel extends Model
{
    ...
    protected $fillable = [
        'title','desc','phone','bounty','delivery_deadline','status','employee_uid','employer_uid','employ_percentage','cancel_at','except_max_at','accept_deadline','created_at','updated_at','room_config','favourite_style','project_position','square','user_type',
     ...
     * @return bool
     */
    static public function employBounty($money,$employ_id,$uid,$code,$type=1,$use_froze_money = false,$action = 1)
    {
        $status = DB::transaction(function () use ($money, $employ_id, $uid, $code, $type,$use_froze_money,$action) {
            //扣除用户的余额,其他支付扣冻结金
            if ($action == 2 || $action == 3 || $action == 7) {
                DB::table('user_detail')->where('uid', '=', $uid)->where('balance_status', '!=', 1)->increment('balance', $money);
            } else {
                if ($use_froze_money) {
                    DB::table('user_detail')->where('uid', '=', $uid)->where('balance_status', '!=', 1)->decrement('frozen_amount', $money);
                } else {
            ...
            //生成财务记录，action 1:发布任务 2:接受任务 3:用户充值 4:用户提现 5:购买增值服务 6:购买用户商品 7:任务失败退款
            $financial = [
                'action' => $action,
                'pay_type' => $type,
                'cash' => $money,
     ...
     * @param $task_id
     */
    static public function employHandle($type,$task_id,$user_id,$api_choose = false)
    {
        $result = false;
        //判断用户是否有资格操作
        ...
        {
            if($api_choose){
                $result = self::where('id',$task_id)->update(['status'=>1,'begin_at'=>date('Y-m-d H:i:s',time()),'employee_uid'=>$user_id]);
            }else{
    ...
    //验收通过稿件
    static public function acceptWork($id,$uid,$platform = true)
    {
        $status = DB::transaction(function() use($id,$uid,$platform){
            //将任务的状态改变成3
