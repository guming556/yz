...
class GoodsModel extends Model
{
    ...
    protected $fillable = [
        ...
        'sales_num', 'comments_num','view_num','is_delete','recommend_text','seo_title','seo_keyword','seo_desc','good_comment','square','house_id','style_id','goods_address'
     ...
     * @param $province 城市
     */
    static public function getWorksList($condition=array())
    {
        $goodsList = GoodsModel::whereRaw('1 = 1');

        //风格筛选
        if(isset($condition['style_id'])){
            $goodsList = $goodsList->where('goods.cate_id',$status);
        }

        //户型筛选
        if(isset($condition['house_id'])){
            $goodsList = $goodsList->where('goods.house_id',$status);
        }

        // TODO 空间是按作品附件的位置来筛选
        if(isset($condition['space_id'])){
            // $goodsList = $goodsList->where('goods.space_id',$status);
        }
        ...
        //     }
        // }
        $goodsList = $goodsList->where('goods.type',1)->where('is_delete',0)
