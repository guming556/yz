...
class WorkModel extends Model
{
    ...
    public $fillable = ['desc','task_id','status','uid','bid_at','created_at','actual_square'];
    ...
    // TODO 这里后期再与上findAll()合成吧
    static function appFindAll($id,$data=array())
    {
        $query = Self::select('work.id as work_id','work.task_id','work.uid','us.name as nickname','a.avatar','a.address','a.experience','a.mobile')
            ->where('work.task_id',$id)->where('work.status',1)->where('forbidden',0);
      
        // TODO 原版
        $data = $query->join('user_detail as a','a.uid','=','work.uid')
            ->join('users as us','us.id','=','work.uid')
            ->get()->toArray();
    ...
    public function workCreate($data)
    {
        $status = DB::transaction(function() use($data){
            ...
            // $work_num = WorkModel::where('task_id',$data['task_id'])->where('status',0)->count();
            if($work==3)
            {   
                // TODO
                // 原版
                // TaskModel::where('id',$data['task_id'])->update(['status'=>4]);
                TaskModel::where('id',$data['task_id'])->update(['status'=>5]);
            }
        });
