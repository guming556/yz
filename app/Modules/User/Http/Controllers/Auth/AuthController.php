...
class AuthController extends IndexController
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->initTheme('auth');
        $this->theme->setTitle('易装网络科技有限公司');
    ...
    public function postLogin(LoginRequest $request)
    {
        ...
        if ($request->get('code') && !\CommonClass::checkCode($request->get('code'))) {
            ...
            if (!UserModel::checkPassword($request->get('username'), $request->get('password'))) {
                ...
                $user = UserModel::where('name', $request->get('username'))->where('user_type',2)->first();
        ...
        $user = UserModel::where('user_type',2)->where('email', $request->get('username'))->orWhere('name', $request->get('username'))->first();
        if ($user && !$user->status) {
