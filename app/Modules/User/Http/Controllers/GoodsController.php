...
class GoodsController extends UserCenterController
{
    ...
    public function getPubGoods()
    {
        ...
        // 获取户型
        $houseCate = HouseModel::select('id', 'name')->where('is_deleted', 0)->get();
// var_dump($houseCate);exit;
        //查询是否开启推荐商品增值工具
     ...
     */
    public function postPubGoods(Request $request)
    {
...
// var_dump($data);exit;
        ...
        if (isset($data['section'])){
            ...
            $desc  = $data['section']['desc'];

            if( count($title) == count($desc) && count($file) == count($desc) ){
                $arrAttachment = array();
