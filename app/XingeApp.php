...
class XingeApp extends Model
{
     ...
     */
    public static function PushTokenAndroid( $content, $token, $custom=[])
    {
        $push = new XingeApp(env('ACCESS_ID_Android'), env('SECRET_KEY_Android'));
        $mess = new Message();
        $mess->setTitle('易装');
...
class Style
{
    /**
     * Style constructor.
     * @param $builderId 本地通知样式，必填
     * @param int $ring 是否响铃，0否，1是
     * @param int $vibrate 是否振动，选填，默认1
     * @param int $clearable 通知栏是否可清除，选填，默认1
     * @param int $nId 通知id，选填。若大于0，则会覆盖先前弹出的相同id通知；若为0，展示本条通知且不影响其他通知；若为-1，将清除先前弹出的所有通知，仅展示本条通知。默认为0
     * @param int $lights 是否呼吸灯，0否，1是，选填，默认1
     * @param int $iconType 默认0，通知栏图标是应用内图标还是上传图标,0是应用内图标，1是上传图标,选填
     * @param int $styleId Web端设置是否覆盖编号的通知样式，默认1，0否，1是,选填
     */
