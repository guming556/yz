...
(function() {
    ...
    LUploader.prototype.init = function() {
        ...
        //_self.trigger.setAttribute('capture', _self.params.capture);
...
//var bbbb=    document.getElementsByClassName('img_container')
        _self.trigger.addEventListener('change', function() {


            //console.log(bbbb.childNodes.length);

            if (!this.files.length) return;
            var files = Array.prototype.slice.call(this.files);
            var currentImageNum = document.getElementsByClassName('LUploader-list')[0].getElementsByTagName('li').length;
            //console.log(currentImageNum);
            if(currentImageNum >= _self.params.maxUpload){
                console.log('最多9张');
                return;
            }
            ...
            files.forEach(function(file, i) {
                ...
                if (!_self.params.multiple) { //假如是单个上传
                    ...
                    currentImageNum++;
                    if (currentImageNum  > _self.params.maxUpload) {
                        console.log('说了最多9张，还传你妹啊');
                        return;
                    }
                    if(currentImageNum > 3){
                        console.log(_self)
                    }
                    //var ul = document.getElementsByClassName('LUploader-list').childNodes;
                    //console.log(ul);
                }
                ...
                EXIF.getData(file, function () {
                    //document.getElementById('Orientation').setAttribute('data-upload-Orientation',EXIF.getTag(this, 'Orientation'));
                });
                ...
                reader.onload = function() {
                    ...
                    //var Orientation = document.getElementById('Orientation').getAttribute('data-upload-Orientation');
                    var Orientation = 0;
                    //alert(Orientation)
                    ...
                    function callback() {
                        var data = _self.compress(img , Orientation);
    ...
    LUploader.prototype.compress = function(img , Orientation) {
    ...
    LUploader.prototype.upload = function(url, obj, basestr, type, li) {
        ...
        xhr.onload = function(e) {
            if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 0) {
                ...
                if(text == '上传成功'){
//console.log(data);
                    var ret_path = data['path'];
                    var uploadInputList = document.getElementById(targetName + '_path_list');
                    var imagePath = document.createElement("input");
                    imagePath.value = ret_path;
                    imagePath.name = targetName + '_path[]';
                    uploadInputList.appendChild(imagePath);
