<h3 class="header smaller lighter blue mg-bottom20 mg-top12">添加辅材包</h3>
<form action="/manage/auxAdd" method="post">
    ...
    <div class="g-backrealdetails clearfix bor-border">
        ...
        <div class="bankAuth-bottom clearfix col-xs-12">
            ...
            <p class="col-md-11">
                <input type="text" name="name" id="name" value="{{empty($agree['name'])?'':$agree['name']}}">
                <input type="hidden" name="id" value="{{empty($agree['id'])?'':$agree['id']}}">
                {{ $errors->first('name') }}
            </p>
        </div>
        ...
        <div class="bankAuth-bottom clearfix col-xs-12">
            ...
            <p class="col-md-11">
                <input type="text" name="aux_price" id="aux_price" value="{{empty($agree['name'])?'':$agree['name']}}">
                <input type="hidden" name="id" value="{{empty($agree['id'])?'':$agree['id']}}">
                {{ $errors->first('name') }}
            </p>
        </div>
        ...
        <div class="bankAuth-bottom clearfix col-xs-12">
            ...
            <p class="clearfix col-md-8">
                <script id="editor" type="text/plain" style="width:;height:300px;" name="content">{!! htmlspecialchars_decode(empty($agree['content'])?'':$agree['content']) !!}</script>
