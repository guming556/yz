...
<div class="widget-header mg-bottom20 mg-top12 widget-well">
    <div class="widget-toolbar no-border pull-left no-padding">
        <ul class="nav nav-tabs" id="a">
            <li @if($user_type == 0) class="active" @endif>
                <a href="{!! url('manage/userList?user_type=0') !!}" title="">[ 未分类 ]</a>
            </li>
            @if(!empty($user_type_list))
                @foreach($user_type_list as $index => $_type)
                    <li @if($user_type == $index) class="active" @endif>
                        <a href="{!! url('manage/userList?user_type='.$index) !!}" title="">[ {!! $_type !!} ]</a>
                    </li>
                @endforeach
            @endif
    ...
    <div class="col-xs-12">
        ...
        <div>
                                    ...
                                    <div class="btn-group">
                                        @if($item->user_type == 1)
                                            <a class="btn btn-xs btn-info"
                                               href="{!! url('manage/userEdit/' . $item->id) !!}">
                                                <i class="fa fa-edit"></i>编辑
                                            </a>

                                            <a class="btn btn-xs btn-success"
                                               href="{!! url('manage/userRecharge/' . $item->id) !!}">
                                                <i class="fa fa-edit"></i>充值
                                            </a>
                                            @if(!empty($item->has_downline))
                                                <a class="btn btn-xs btn-info"
                                                   href="{!! url('manage/getUserDownline/' . $item->id) !!}">
                                                    <i class="fa"></i>查看发展的用户
                                                </a>
                                            @endif
                                        @else
                                            <a class="btn btn-xs btn-info"
                                               href="{!! url('manage/workerEdit/' . $item->id) !!}">
                                                <i class="fa fa-edit"></i>编辑
                                            </a>
                                        @endif
                                        <a id="modal-684890" href="#modal-container-684890" role="button"
                                           class="btn btn-xs" data-toggle="modal" onclick="generate_invit_url(this)"
                                           altuid="{{$item->id}}">生成邀请链接</a>
                                        ...
                                        @elseif($item->status == 2)
                                            <a class="btn btn-xs btn-success"
                                               href="{!! url('manage/handleUser/' . $item->id . '/enable') !!}">
                                                <i class="fa fa-check"></i>启用
                                            </a>
                                        @endif
                                        @if(($item->user_type == 2 || $item->user_type == 3 || $item->user_type == 4) && !empty($item->is_agree))
                                            <a class="btn btn-xs btn-warning"
                                               href="{!! url('manage/workGoodsList/' . $item->id) !!}">
                                                <i class="ace-icon fa fa-search bigger-120">作品列表</i>
                                            </a>
                                        @endif

                                        @if($item->user_type != 1  && empty($item->is_agree))
                                            <a class="btn btn-xs btn-success"
                                               href="{!! url('manage/realnameAuthList') !!}">
                                                <i class="ace-icon fa fa-search bigger-120">去实名认证</i>
                                            </a>
                                        @endif
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <button id="all_test_able" class="btn btn-primary btn-default btn-round">一键启用测试账号
                    </button>
                </div>
                <button id="all_test_disable" class="btn btn-primary btn-default btn-round">一键禁用测试账号
                </button>
